...
public class GatherResourceAbilityTest {
	...
	public void test() {
		...
		EntityManager.INSTANCE.addNewPCE("Headquarter", player, new Position(
				10.5f, 15.5));
		EntityManager.INSTANCE.addNewPCE("Barracks", player, new Position(
				35.5f, 15.5));
		EntityManager.INSTANCE.addNewNPCE("Resource",
				new Position(15.5f, 15.5f));
		EntityManager.INSTANCE.update(1);
		Worker worker = (Worker) EntityManager.INSTANCE
				.getPCEAtPosition(new Position(2.5f, 15.5f));
		Resource res = (Resource) EntityManager.INSTANCE
				.getNPCEAtPosition(new Position(15.5f, 15.5f));
		...
		while (player.getResources() != GatherResourceAbility.RESOURCE_CARRIED_AMOUNT
				* 2 + Player.RESOURCE_START_AMOUNT) {
			ab.update(.5f);
			move.update(.5f);
			if (counter == 12) {
				EntityManager.INSTANCE.addNewPCE("Headquarter", player,
						new Position(8.5f, 15.5));
				EntityManager.INSTANCE.update(1);
			}
			counter++;
			try {
