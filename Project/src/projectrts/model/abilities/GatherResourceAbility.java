...
public class GatherResourceAbility extends AbstractAbility implements
	...
	public void useAbility(Position target) {
		if (EntityManager.INSTANCE.getNPCEAtPosition(target) instanceof Resource) {
			this.targetResource = (Resource) EntityManager.INSTANCE
					.getNPCEAtPosition(target);
	...
	private void findDepositStructure() {
		List<IPlayerControlledEntity> entities = EntityManager.INSTANCE
				.getEntitiesOfPlayer(entity.getOwner());
			...
			if (e instanceof AbstractStructure) {
				...
				if (struct.isDeposit()) {
					if (depositStructure == null) {
						...
						if (Position.getDistance(entity.getPosition(),
								e.getPosition()) < Position.getDistance(
								entity.getPosition(),
								depositStructure.getPosition())) {
							// If e is closer to unit than saved
							// depositStructure
							depositStructure = struct;
						}
					}
	...
	private void updateMineResource(float tpf) {
		if (inRange(targetResource)) {
			// Check cooldown and mine resource or reduce cooldown as
			// appropriate.
			moveAbility.setFinished(true);
			if (miningCooldown <= 0) {
